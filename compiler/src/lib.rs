...
mod tests {
    ...
    fn access_compile_parameters_select_shared_or_mutable_borrowing() {
        let source = "let inspect: <a: access>(value: Borrow<a><i32>): i32 = {  value }\n\
             let borrow_value: <a: access, r: region, t: type>(value: Borrow<a><r><t>): Borrow<a><r><t> = {  borrow<a>(value) }\n\
             let main: (): i32 = { \n\
             let mut left = 20\n\
             let right = 22\n\
             let mut third = 0\n\
    ...
    fn arrays_unsize_to_region_bound_slice_borrows() {
        let ir = compile_source(
            "let Slice = core.memory.Slice\n\
             let view: <r: region>(values: Borrow<r><Array<i32><3>>): Borrow<r><Slice<i32>> = {\n\
             borrow(values)\n\
             }\n\
             let main: (): i32 = { \n\
             let values = [40, 1, 1]\n\
    ...
    fn slice_methods_preserve_length_and_element_borrow_access() {
        let ir = compile_source(
            "let Slice = core.memory.Slice\n\
             let inspect: <r: region>(values: Borrow<r><Slice<i32>>): i32 = {\n\

...
#[test]
fn parses_name_side_declaration_signatures() {
    let program = parse(
        "let Identity<T: type>: type = T\n\
         let Cell<T: type> = struct { value: T }\n\
         let identity<T: type>(value: T): T = { value }\n\
         let brace_local<T: type>(value: T): T = { value }\n",
    )
    .expect("named compile-time parameters must precede `=`");
    for item in &program.items {
        match item {
            Item::Function(function) => assert_eq!(function.compile_groups[0][0].name, "T"),
            ...
            _ => panic!("expected a generic declaration"),
        }
    }
...
#[test]
fn named_callable_signatures_precede_equals() {
    let program = parse(
        "let typed(p: i32) = { \n  let value: i32 = p\n  value\n}\n\
    ...
    .expect("named callable signatures precede equals");
...
#[test]
fn parses_name_attached_trait_and_effect_members_without_semicolons() {
    parse(
...
fn parses_access_parameters_in_borrow_modes_types_and_expressions() {
    let program = parse(
        "let identity<a: access, r: region, t: type>(value: Borrow<a><r><t>): Borrow<a><r><t> = { borrow<a>(value) }\n",
    )
    .unwrap();
    let Item::Function(function) = &program.items[0] else {
        panic!("expected function");

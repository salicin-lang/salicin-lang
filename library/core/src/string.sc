...
extend<str> {
  ...
  let is_char_boundary(self: Borrow<self>)
    (index: u64): bool = {
    let length = self.len()
  ...
  let scalar_at(self: Borrow<self>)
...
extend<String> {
  ...
  let reserve(self: Borrow<mut><self>)
    (additional: u64): () = {
  ...
  let truncate(self: Borrow<mut><self>)
    (new_length: u64): bool = {
  ...
  let substring(self: Borrow<self>)

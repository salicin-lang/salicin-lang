...
let text_equal = {
  (left: Borrow<str>, right: Borrow<str>): bool =>
...
let text_find = {
  (value: Borrow<str>, needle: Borrow<str>): core.Option<u64> =>
...
let utf8_error_offset = { (bytes: Borrow<Slice<u8>>): core.Option<u64> =>
  ...
  while(index < length) {
    ...
    if(first <= ascii_max) {
      ...
      if(first >= two_min && first <= two_max) {
        if(index + one >= length ||
...
extend(str) {
  ...
  let find = {
    (self: Borrow<self>)
    (needle: Borrow<str>): core.Option<u64> =>
  ...
  let scalar_at = {
    (self: Borrow<self>)
    (index: u64): core.Option<UnicodeScalar> =>
    let mut values = self.scalars()
...
extend(UnicodeScalar) {
  ...
  let from_u32 = {
    (value: u32): core.Option<UnicodeScalar> =>
...
let string_allocate = {
  (capacity: u64): Ptr<mut><u8> =>
  unsafe {
    raw_alloc<u8>(capacity, 1)
  }
}

let string_deallocate = {
  (data: Ptr<mut><u8>, capacity: u64): () =>
  unsafe {
...
let string_reserve = {
  (value: Borrow<mut><String>, additional: u64): () =>
...
let string_push_byte = {
  (value: Borrow<mut><String>, byte: u8): () =>
  unsafe {
...
let string_is_char_boundary = {
  (value: Borrow<String>, index: u64): bool =>
...
extend(String) {
  ...
  let from_unicode_scalar = {
    (value: UnicodeScalar): String =>
  ...
  let substring = {
    (self: Borrow<self>)
    (start: u64, end: u64): core.Option<String> =>

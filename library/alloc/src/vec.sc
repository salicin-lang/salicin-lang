...
let vec_allocate: <T: type>
  (capacity: u64): Ptr<mut><T> = {
  unsafe {
...
let vec_deallocate: <T: type>
  (pointer: Ptr<mut><T>, capacity: u64): () = {
  unsafe {
...
let vec_at: <a: access, r: region, T: type>
  (values: Borrow<a><r><Vec<T>>)
  (index: u64): Borrow<a><r><T> = {
...
let vec_pop: <T: type>
  (values: Borrow<mut><Vec<T>>): Option<T> = {
  if(values.length == 0) {
    Option<T>.None
  } else: {
    values.length = values.length - 1
...
let vec_append: <T: type>
  (values: Borrow<mut><Vec<T>>)
  (other: Borrow<mut><Vec<T>>): () = {
...
extend<Vec<T>><requires: T is Copyable> {
  ...
  let copy_from: (self: Borrow<mut><self>)
    (source: Borrow<Slice<T>>): () = {
    let values = self.as_slice<mut>()

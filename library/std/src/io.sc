...
pub let arguments_bytes = { with<io>(): alloc.vec.Vec<ProcessArgument> =>
    let count = argument_count()
...
extend(OpenOptions) {
  ...
  let with_read = { (move self)(enabled: bool): OpenOptions =>
  ...
  let with_write = { (move self)(enabled: bool): OpenOptions =>
  ...
  let with_append = { (move self)(enabled: bool): OpenOptions =>
  ...
  let with_truncate = { (move self)(enabled: bool): OpenOptions =>
  ...
  let with_create = { (move self)(enabled: bool): OpenOptions =>
  ...
  let with_create_new = { (move self)(enabled: bool): OpenOptions =>
...
extend(File) {
  ...
  let flush = { with<io>
      (self: Borrow<self>)(): core.Result<IoError><()> =>
      let mut failure: i32 = 12
    let mut raw_code: i32 = 0
    let status = unsafe {
      host_flush(
  ...
  let close = { with<io>(move self)(): core.Result<IoError><()> =>

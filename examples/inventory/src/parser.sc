...
test("decimal parser accepts signed input") {
  ...
  match(decimal(view)) { Ok(value) => std.test.assert(value == -17), Err(_) => std.test.fail("expected parsed -17"),
  }
}
...
test("decimal parser rejects trailing text") {
  ...
  let view = input.as_str()
  match(decimal(view)) {
    Ok(_) => std.test.fail("expected InvalidDigit"), Err(error) => do {
      match(error.kind()) { InvalidDigit => (), _ => std.test.fail("expected InvalidDigit"),
      }
    },
  }
}

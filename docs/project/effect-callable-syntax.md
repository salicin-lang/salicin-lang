...
An effectful declaration places its signature before `=` and its implementation
...
The final colon introduces the declaration result. A pure named callable uses
...
The colon before `with` is always required in a named declaration, including
after compile-time parameter groups, as in `apply<e: effects>: with<e>` above.
...
Named callable signatures precede `=`, while anonymous/local closures retain
the brace form `{ (...) => ... }`. Callable types remain `with<e>(T): R`
without braces.

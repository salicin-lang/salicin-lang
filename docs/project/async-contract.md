...
fallthrough transfers them to `IterationSkip`; a value-producing `break` transfers its value to
`LoopExit`. A `while` condition that becomes false is the unit-valued break path.
...
2. `Ready(IterationSkip(carry))` destroys the completed child, constructs the next iteration in the same
   child slot from `carry`, and polls it immediately;
3. `Ready(LoopExit(output))` destroys the completed child, marks the parent completed, and returns
   `Ready(output)`.

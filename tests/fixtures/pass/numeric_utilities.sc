...
let sign_checks = {
  ...
  let zero: i32 = 0
  let positive: u32 = 1
  let negative_ok = match(negative.sign()) { Negative => true, _ => false,
  }
  let zero_ok = match(zero.sign()) { Zero => true, _ => false,
  }
  let positive_ok = match(positive.sign()) { Positive => true, _ => false,
  }
...
let conversion_checks = {
  ...
  is_some_u8(narrow.checked_into<Output: u8>(), 255) &&
    ...
    match(too_large.checked_into<Output: i16>()) { Some(_) => false, None => true,
  }
}
...
let wide_checks = {
  ...
  let unsigned_to_signed = match(maximum.checked_into<Output: i128>()) { Some(_) => false, None => true,
  }
  let pointer_to_signed = match(pointer_maximum.checked_into<Output: isize>()) { Some(_) => false, None => true,
  }

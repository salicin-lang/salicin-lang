...
let outer = {
  (move action: with<ask>(): i32, abandon: bool): i32 =>
  ask.handle {
    value: {
      (resume) => if(abandon) { 40 } else: { resume(20) }
    },
    action: { run(action) },
  }
}
...
let run_input = {
  (move action: with<ask>(i32): i32): i32 =>
  ask.handle {
    value: { (resume) => resume(10) },
    action: { apply_input(11, action) },
  }
}

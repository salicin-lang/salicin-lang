...
let abandon: with<std.io.io>
  ...
  match std.io.open(path)(std.io.OpenOptions.read_only())
    { Ok(value) -> () }
    { Err(_) -> () }
}
...
let main: with<std.io.io>(): i32 = {
  ...
  let input = match std.io.open(view)(std.io.OpenOptions.read_only())
    { Ok(value) -> value }
    { Err(_) -> return(1) }
  match input.close()
    { Ok(_) -> return(2) }
...
fn native_file_owners_support_options_seek_flush_limits_and_close() {
  ...
  match std.io.read_file(path_view)(2)
    ...
    { Ok(_) -> return(4) }
  let mut output = match std.io.open(path_view)(std.io.OpenOptions.append_only())
    { Ok(value) -> value }
    { Err(_) -> return(5) }
  ...
  match output.close()
    { Err(_) -> return(8) }
    { Ok(_) -> () }
  match std.io.open(path_view)(std.io.OpenOptions.create_new())
    { Err(error) ->
      match error.kind()
        { AlreadyExists -> () }
        { _ -> return(9) }
    ...
    { Ok(_) -> return(10) }
  match std.io.open(missing_view)(std.io.OpenOptions.read_only())
    { Err(error) ->
      match error.kind()
        { NotFound -> () }
        { _ -> return(11) }
  ...
  match std.io.open(nul_view)(std.io.OpenOptions.read_only())
    { Err(error) ->
      match error.kind()
        { InvalidInput -> () }
        { _ -> return(21) }
    ...
    { Ok(_) -> return(22) }
  let mut input = match std.io.open(path_view)(std.io.OpenOptions.read_only())
    { Ok(value) -> value }
    { Err(_) -> return(13) }
